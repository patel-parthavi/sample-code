...
if ( ( class_exists( 'LearnDash_Settings_Metabox' ) ) && ( ! class_exists( 'Metabox_Topic_Group_Drip_Settings_Addon' ) ) ) {

	/**
	 * Class Metabox_Topic_Group_Drip_Settings_Addon
	 */
	class Metabox_Topic_Group_Drip_Settings_Addon extends \LearnDash_Settings_Metabox {
		...
		public function load_settings_fields() {
			...
			$reload = esc_attr__( 'Note: Page reloads after Save/Remove date', 'uncanny-pro-toolkituncanny-pro-toolkituncanny-pro-toolkituncanny-pro-toolkituncanny-pro-toolkituncanny-pro-toolkituncanny-pro-toolkit' );
			ob_start();
			...
			<table id="uo-ld-group-drip" class="striped celled table" style="width:100%">
				...
				foreach ( $groups as $group ) {
					if ( $group && is_object( $group ) ) {
						...
						$date = get_post_meta( $post->ID, stripslashes( '\drip_topic_group\AddonDripTopicByGroup' ) . '-' . $group->ID, true );
						// Add tha ( date ) after group name on selection if exists
						if ( $date ) {
							if ( is_array( $date ) ) {
								$date   = \drip_topic_group\AddonDripTopicByGroup::reformat_date( $date );
								$u_date = $date;
								$date   = learndash_adjust_date_time_display( $date );
							}

							if ( \drip_topic_group\AddonDripTopicByGroup::is_timestamp( $date ) ) {
								$u_date = $date;
								$date   = \drip_topic_group\AddonDripTopicByGroup::adjust_for_timezone_difference( $date );
							}
						}
						?>
							...
							<td>
								...
								<div class="ld_date_selector">
									...
									<input type="number" placeholder="DD" min="1" max="31"
										   ...
										   name="day" value="<?php echo date( 'd', $date ) ?>" size="2" maxlength="2"
										   autocomplete="off">,
									...
									<input type="number" placeholder="YYYY"
										   ...
										   name="year" value="<?php echo date( 'Y', $date ) ?>" size="4" maxlength="4"
									...
									<input type="number" min="0" max="23" placeholder="HH"
										   ...
										   name="hour" value="<?php echo date( 'H', $date ) ?>" size="2" maxlength="2"
										   autocomplete="off">:
									...
									<input type="number" min="0" max="59" placeholder="MN"
										   ...
										   name="minute" value="<?php echo date( 'i', $date ) ?>" size="2" maxlength="2"
										   autocomplete="off">
								</div>
							</td>
	...
	add_filter(
		...
		function ( $metaboxes = array() ) {
			if ( ( ! isset( $metaboxes['Metabox_Topic_Group_Drip_Settings_Addon'] ) ) ) {
				$metaboxes['Metabox_Topic_Group_Drip_Settings_Addon'] = \Metabox_Topic_Group_Drip_Settings_Addon::add_metabox_instance();
			}
